...
public final class RemoteKeyDao_Impl implements RemoteKeyDao {
  ...
  public RemoteKeyDao_Impl(RoomDatabase __db) {
    ...
    this.__insertionAdapterOfRemoteKeys = new EntityInsertionAdapter<RemoteKeys>(__db) {
      ...
      public String createQuery() {
        return "INSERT OR REPLACE INTO `RemoteKeys` (`repoId`,`prevKey`,`nextKey`) VALUES (?,?,?)";
      }

      @Override
      public void bind(SupportSQLiteStatement stmt, RemoteKeys value) {
        if (value.getRepoId() == null) {
          stmt.bindNull(1);
        } else {
          stmt.bindString(1, value.getRepoId());
        }
  ...
  @Override
  public Object clearRemoteKeys(final Continuation<? super Unit> continuation) {
    return CoroutinesRoom.execute(__db, true, new Callable<Unit>() {
      @Override
      public Unit call() throws Exception {
  ...
  @Override
  public Object remoteKeysId(final int id, final Continuation<? super RemoteKeys> continuation) {
    final String _sql = "SELECT * FROM remotekeys WHERE repoId = ?";
      ...
      public RemoteKeys call() throws Exception {
        ...
        try {
          ...
          if(_cursor.moveToFirst()) {
            final String _tmpRepoId;
            if (_cursor.isNull(_cursorIndexOfRepoId)) {
              _tmpRepoId = null;
            } else {
            ...
            _result = new RemoteKeys(_tmpRepoId,_tmpPrevKey,_tmpNextKey);
          } else {
            _result = null;
          }
          return _result;
